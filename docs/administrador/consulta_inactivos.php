...
<html lang="en">
    ...
    function ConfirmarHabilitar() {
      var confirmar = confirm("¿Realmente desea habilitar a esta persona? ");
      if (confirmar) {
        return true;
      } else {
        return false;
                    ...
                    <div class="tile-body">
                      <table class="table table-hover table-bordered " id="sampleTable">
                        ...
                        <tbody>
                          <?php
                          try{
                            $sql="SELECT id_persona,nombre,telefono,nombre_rol,login, 'Inactivo' AS estado_pers FROM  persona inner join rol on (id_rol=fk_rol) where  estado_pers=0" ;
                            $resul=$base->prepare($sql);
                            $resul->execute(array());
                            while($consulta=$resul->fetch(PDO::FETCH_ASSOC)){
                              ?>                              
                                ...
                                <th>
                                  <span id="fk_rol<?php echo$consulta['id_persona'];?>"><?php echo$consulta['nombre_rol'];?></span>
                                </th>
                                <th>
                                ...
                                <td><a class="btn btn-primary edit " style="width:100px; margin-left: 20px;" onclick='return ConfirmarHabilitar();'href="php/habilitar.php?cod=<?php echo $consulta['id_persona']?>"><i class='material-icons' title=Actualizar Registro>Habilitar</i></a></button></td>
                              </tr>
