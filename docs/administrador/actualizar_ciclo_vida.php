...
<html lang="en">
    <head>
       ...
       <script type="text/javascript">
        ...
        {
          var confirmar = confirm("¿Realmente desea actualizar? ");
          if (confirmar)
          {
            return true;
          } 
...
<!-- FINAL ASIDE----------------------------------------------------------------------------------------------------------------------------->
  ...
  <main class="app-content">
    <div>
      <div class="app-title">
        <h1><i class="fa fa-dashboard"></i>Actualizar datos </h1>
      </div>
      <div>
     <form action="" method="POST">
          ...
          try {
            ...
            inner join tipo_alimentacion on (fk_tipo_alimentacion=id_tipo_alimentacion) WHERE id_ciclo_vida= " . $_REQUEST['enviar'] . " ";
            ...
            while ($consulta = $resultado->fetch(PDO::FETCH_ASSOC)) {
              ...
              <div class="row">
                <div class="col-md-6">
                    <div>
                        ...
                        <td> <input class="form-control" type="number" name="semana" value="<?php echo $consulta['semana'] ?>"></td>
                    </div>
                    <div>
                        <label for="">Consumo diario</label>
                        <td> <input class="form-control" type="text" name="diario" value="<?php echo $consulta['consumo_diario'] ?>"> </td>
                    </div>
                    <div>
                        <label for="">Consumo semanal</label>
                        <td> <input class="form-control" type="text" name="semanal" value="<?php echo $consulta['consumo_semanal'] ?>"> </td>
                    </div>
                </div>
                <div class="col-md-6">
                    <div>
                        <label for="">Peso minimo</label>
                        <td> <input class="form-control" type="text" name="minimo" value="<?php echo $consulta['peso_min'] ?>"> </td>
                    </div>
                     <div>
                        <label for="">Peso máximo</label>
                        <td> <input class="form-control" type="text" name="maximo" value="<?php echo $consulta['peso_max'] ?>"> </td>
                    </div>
                    <div>
                        <label for="exampleInputPassword1">Alimentación</label>
                        <select class="form-control" name="alimentacion" id="" class="form_control">
                            <?php
                            try {
                       
                                $query = "SELECT * from tipo_alimentacion ";
                                $data = $base->prepare($query);
                                $data->execute();
                                while ($row = $data->fetch(PDO::FETCH_ASSOC)) {
                                    echo '<option value=" '.$row ['id_tipo_alimentacion'].'">'.$row ['nombre_alimentacion'].'</option>';
                                }
                            } 
                            catch (exception $e) {
                                die('error ->' . $e->getMessage());
              ...
              <div class="row">
                  <div class="col-md-12">
                        ...
                        <select class="form-control" name="especie" id="" class="form_control">
                            <?php
                            try {
                       
                                $query = "SELECT * from tipo_especie ";
                                $data = $base->prepare($query);
                                $data->execute();
                                while ($row = $data->fetch(PDO::FETCH_ASSOC)) {
                                    echo '<option value=" '.$row ['id_tipo_especie'].'">'.$row ['nombre_especie'].'</option>';
                                }
                            } 
                            catch (exception $e) {
                                die('error ->' . $e->getMessage());
              ...
              <div>
                <td><input  class="btn btn-primary"  type="submit" name="btn1"  value="Actualizar" onclick='return ConfirmarUpdate();' ></td>
                
              </div>
            <?php
            }
          } catch (excepcion $ex) {
            die('Error :' . $y->getMassage());
          }
...
if(isset($_POST['btn1'])){
    ...
    try
    {
        $sql="UPDATE ciclo_vida SET semana=:sem ,consumo_diario=:dia,consumo_semanal=:semanal,peso_min=:min,
         peso_max=:max,fk_tipo_especie=:espe,fk_tipo_alimentacion=:ali WHERE id_ciclo_vida=".$_REQUEST['enviar']." ; ";

        $resultado=$base->prepare($sql);

        $resultado->execute(array(":sem"=>$semana ,":dia"=>$diario,":semanal"=>$semanal, ":min"=>$minimo,":max"=>$maximo,":espe"=>$especie,
            "ali"=>$alimentacion)); 

            ?>
            <script type="text/javascript">window.location="consulta_ciclo_vida.php"</script>
            <?php
    }
    catch(exception $e){
        die('error:' . $e->getMessage());
