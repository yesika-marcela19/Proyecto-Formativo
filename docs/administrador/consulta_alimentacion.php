...
<main class="app-content">
  <div class="app-title">
    <div>
      <h1><i class="fa fa-th-list"></i> Alimentación registrada</h1>
  </div>
...
<div  class="row d-flex justify-content-center" >
        ...
        <tbody>
          <?php
          try{
            $sql="SELECT  nombre,fecha,cantidad,descripcion,lote, 'Activo' AS estado_lote FROM  actividad INNER JOIN persona ON (fk_persona=id_persona)  WHERE fk_tipo_actividad=5";
            $resul=$base->prepare($sql);
            $resul->execute(array());
            while($consulta=$resul->fetch(PDO::FETCH_ASSOC)){
              ?>
              <tr>                            
                ...
                <td><?php echo$consulta['estado_lote'];?></td>            
            </tr>
            <?php
        }
    }
...
<td>
    <a href="../reportes/alimentacion.php" class="btn btn-primary"> Descargar </a>
</td>
</div>
</div>
</div>
