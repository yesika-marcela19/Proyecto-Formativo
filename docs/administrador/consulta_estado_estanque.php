...
<html lang="en">
    <head>
        ...
        function ConfirmarUpdate1() 
        {
          var confirmar = confirm("¿Realmente desea deshabilitar este estanque? ");
            if (confirmar) {
              return true;
            } 
            else {
        ...
        <script type="text/javascript">
        function ConfirmarUpdate2() 
        {
          var confirmar = confirm("¿Realmente desea habilitar este estanque? ");
            if (confirmar) {
              return true;
            } 
            else {
...
<!-- FINAL ASIDE----------------------------------------------------------------------------------------------------------------------------->
    ...
    <main class="app-content">
              ...
              <div class="app-title">
                <div>
                  <h1><i class="fa fa-th-list"></i> Estanque </h1>
                </div>
                    ...
                    <div class="tile-body">
                      <table class="table table-hover table-bordered " id="sampleTable">
                        ...
                        <tbody>
                          <?php
                          try{
                            $sql="SELECT id_estanque AS Estanque,latitud AS Latitud,longitud AS Longitud,area AS Area,capacidad AS Capacidad,IF(`estado_estanque`=0,'Inactivo','Activo') as estado_estanque from estanque" ;
                            $resul=$base->prepare($sql);
                            $resul->execute(array());
                            while($consulta=$resul->fetch(PDO::FETCH_ASSOC)){
                              ?>   
                              <tr>                           
                                <th>
                                      <span><?php echo$consulta['Estanque'];?></span>
                                </th>

                                <th>
                                  <span><?php echo$consulta['Latitud'];?></span>
                                </th>
                                <th>
                                  <span><?php echo$consulta['Longitud'];?></span>
                                </th>
                                <th>
                                  <span><?php echo$consulta['Area'];?></span>
                                </th>
                                <th>
                                  <span><?php echo$consulta['Capacidad'];?></span>
                                </th>
                                <th>
                                  <span><?php echo$consulta['estado_estanque'];?></span>
                                </th>
                                  <?php
                                    $estado = $consulta['estado_estanque'];
                                  ?>
                                <td>
                                  <?php
                                      if($estado == 'Activo')
                                      {
                                        ?>
                                        <a class="btn btn-secondary  edit" style="width:120px; margin-left: 30px;" onclick='return ConfirmarUpdate1();'href="php/desabilitar_estanque.php?cod=<?php echo $consulta['Estanque']?>"><i class='material-icons' title=Actualizar Registro>Deshabilitar</i></a>
                                        <?php
                                      }
                                      else 
                                      {
                                        ?>
                                         <a class="btn btn-primary edit" style="width:120px; margin-left: 30px;" onclick='return ConfirmarUpdate2();'href="php/habilitar_estanque.php?cod=<?php echo $consulta['Estanque']?>"><i class='material-icons' title=Actualizar Registro>Habilitar</i></a>
                                        <?php
                                      }
                                  ?>
