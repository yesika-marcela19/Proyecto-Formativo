...
<html lang="en">
    <head>
       ...
       <script type="text/javascript">
        ...
        {
          var confirmar = confirm("¿Realmente desea deshabilitar a esta persona? ");
          if (confirmar) {
            return true;
          } 
          else 
                    ...
                    <div class="tile-body">
                      <table class="table table-hover table-bordered " id="sampleTable">
                        ...
                        <tbody>
                          <?php
                          try{
                            $sql="SELECT id_persona,nombre,telefono,nombre_rol,login,`estado_pers`=1,'Activo'  AS estado_pers FROM  persona inner join rol on (id_rol=fk_rol) where fk_rol=1 AND estado_pers=1" ;
                            $resul=$base->prepare($sql);
                            $resul->execute(array());
                            while($consulta=$resul->fetch(PDO::FETCH_ASSOC)){
                              ?>   <tr>                           
                                <th>
                                  <span><?php echo$consulta['id_persona'];?></span>
                                </th>

                                <th>
                                  <span><?php echo$consulta['nombre'];?></span>
                                </th>
                                <th>
                                  <span><?php echo$consulta['telefono'];?></span>
                                </th>
                                <th>
                                  <span><?php echo$consulta['nombre_rol'];?></span>
                                </th>
                                <th>
                                  <span><?php echo$consulta['estado_pers'];?></span>
                                </th>
                                <td ><a class="btn btn-secondary edit" style="width:100px;" onclick='return ConfirmarUpdate();' href="php/desabilitar_admin.php?cod=<?php echo $consulta['id_persona']?>"><i class='material-icons' title=Actualizar Registro>Deshabilitar</i></a></button></td>
                                
                                <td ><a class="btn btn-primary edit" style="width:100px;" href="actualizar_administrador.php?enviar=<?php echo $consulta['id_persona']?>"><i class='material-icons' >Actualizar</i></a></td>
